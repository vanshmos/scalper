...
def migrate_trades_to_capital_based(db_path: str = "/app/data/trades.db", capital: float = 100000.0, leverage: int = 10) -> Dict:
    """
    Migrate existing trades to use capital-based P&L calculations with leverage.
    Recalculates pnl_absolute, roi_percent, and max_favorable/max_adverse.
    
    Args:
        db_path: Path to SQLite database
        capital: Base capital ($100,000 default)
        leverage: Leverage multiplier (10x default)
    
    Returns:
        Dict with migration results
    """
    try:
        position_size = capital * leverage  # $1M with 10x leverage
        ...
        for row in rows:
            ...
            # Recalculate P&L based on position size (with leverage)
            # pnl_absolute = (pnl_percent / 100) * position_size
            new_pnl = (pnl_percent / 100) * position_size if pnl_percent else 0
            
            # ROI% = P&L / Capital (return on actual capital, not position)
            roi_percent = (new_pnl / capital) * 100 if capital else 0
            
            # Recalculate MFE/MAE based on position size
            if old_mfe and entry_price:
                ...
                if abs(old_mfe) < entry_price * 0.1:  # Less than 10% of entry = likely price diff
                    new_mfe = (old_mfe / entry_price) * position_size
                else:
                    # Already in dollars but needs leverage adjustment
                    new_mfe = old_mfe * leverage
            else:
                new_mfe = 0
                
            if old_mae and entry_price:
                if abs(old_mae) < entry_price * 0.1:
                    new_mae = (old_mae / entry_price) * position_size
                else:
                    new_mae = old_mae * leverage
            else:
                new_mae = 0
            ...
            cursor.execute("""
                UPDATE trades 
                SET pnl_absolute = ?, roi_percent = ?, max_favorable = ?, max_adverse = ?
                WHERE id = ?
            """, (round(new_pnl, 2), round(roi_percent, 4), round(new_mfe, 2), round(new_mae, 2), trade_id))
        ...
        logger.info(f"Migrated {updated_count} trades to capital-based P&L (${capital:,.0f} @ {leverage}x leverage)")
